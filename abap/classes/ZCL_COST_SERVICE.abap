...
CLASS zcl_cost_service DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! 프로젝트 실적원가 갱신 (CO → PS 연계)
    METHODS update_project_actual_cost
      IMPORTING iv_project_id TYPE n.
...
CLASS zcl_cost_service IMPLEMENTATION.
  ...
  METHOD create_cost_entry.
    ...
    "! 금액 자동계산: AMOUNT = QUANTITY * UNIT_PRICE
  ...
  METHOD delete_cost_entry.
    ...
    "! 삭제 후 실적원가 재계산
    update_project_actual_cost( lv_project_id ).
  ENDMETHOD.
